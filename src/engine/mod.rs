...
pub trait Engine {
    ...
    ///
    /// This will only be called when [`Engine::dirty`] is [`true`].
    fn wl_buffer(&mut self) -> Option<&WlBuffer>;

    // TODO: wl_buffer alternative, just testing with Servo.
    //  => Combine and attach from within webkit?
    fn todo(&mut self, surface: &WlSurface) -> bool {
        match self.wl_buffer() {
            Some(buffer) => {
